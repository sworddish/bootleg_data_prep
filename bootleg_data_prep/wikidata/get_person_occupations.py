...
OCCUPATION = 'P106' #it's mainly for person 
#INSTANCE_OF = 'P31'
#SUBCLASS_OF = 'P279'
# Get occupation (for people)
TYPE_PIDS = {OCCUPATION}
...
def load_entity_file(message):
    ...
    out_f = open(os.path.join(out_dir, f"_occupation_out_{job_index}.json"), "w", encoding='utf8')
...
def merge_and_save(out_dir, qid_to_title):
    ...
    in_files = glob(os.path.join(out_dir, f"_occupation_out_*.json"))
    ...
    with open(os.path.join(out_dir, 'occupation_freqs.json'), 'w', encoding='utf8') as out_file:
...
def write_types(out_dir, type_list, qid_to_title):
    ...
    with open(os.path.join(out_dir, 'occupationqid2typeqid.json'), 'w', encoding='utf8') as out_file:
        json.dump(typeqid2qid, out_file, ensure_ascii=ENSURE_ASCII)

    with open(os.path.join(out_dir, 'occupationeqid2title.json'), 'w', encoding='utf8') as out_file:
